...
import { X, BookOpen, Globe, Image, Save } from 'lucide-react';
...
const AddCategoryModal: React.FC<AddCategoryModalProps> = ({ isOpen, onClose, onCategoryAdded }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await mockCategoriesAPI.createCategory(categoryData);
  ...
  const handleLanguageChange = (lang: string, checked: boolean) => {
    setCategoryData(prev => ({
      ...prev,
      allowed_lang: checked 
        ? [...prev.allowed_lang, lang]
        : prev.allowed_lang.filter(l => l !== lang)
    }));
  };
                ...
                <div className="mb-4">
                  <label className="flex items-center space-x-3">
                    <input
                      type="checkbox"
                      name="play_mode"
                      checked={categoryData.play_mode}
                      onChange={handleInputChange}
                      className="w-4 h-4 text-green-600 focus:ring-green-500 border-gray-300 rounded"
                    />
                    <span className="text-sm font-medium text-gray-700">Enable Play Mode</span>
                  </label>
                </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    {[
                      ...
                      <label key={lang.code} className="flex items-center space-x-2">
                        <input
                          type="checkbox"
                          checked={categoryData.allowed_lang.includes(lang.code)}
